...
export class ScheduleRepository {
     ...
     */
    public static async dispenseDoseAtomically(
        client: PoolClient,
        scheduleId: string,
        ...
        const checkIdempotency = await client.query(
            `SELECT id FROM schedules WHERE idempotency_key = $1 LIMIT 1`,
            [idempotencyKey]
        );
        ...
        const scheduleRes = await client.query(
            `SELECT id, compartment_id, scheduled_time, dose_quantity, status 
       FROM schedules 
       WHERE id = $1 FOR UPDATE NOWAIT`,
        ...
        const compartmentRes = await client.query(
            `SELECT id, total_tablets, threshold, daily_dose, low_stock_notified 
       FROM compartments 
       WHERE id = $1 FOR UPDATE`,
            [schedule.compartmentId]
        );
       ...
       WHERE id = $4`,
            [newTotalTablets, lowStockNotified, currentTime, schedule.compartmentId]
        );
       ...
       WHERE id = $4`,
            [ScheduleStatus.DISPENSED, idempotencyKey, currentTime, scheduleId]
        );
        ...
        await client.query(
            `INSERT INTO audit_logs (schedule_id, compartment_id, event_type, details, created_at) 
       VALUES ($1, $2, $3, $4, $5)`,
            [scheduleId, schedule.compartmentId, EventType.DISPENSE_SUCCESS, JSON.stringify(dispenseLogDetails), currentTime]
        );

        // 7. Insert additional audit logs if alerts triggered
        if (needsLowStockAlert) {
            await client.query(
                `INSERT INTO audit_logs (compartment_id, event_type, details, created_at) 
         VALUES ($1, $2, $3, $4)`,
                [schedule.compartmentId, EventType.THRESHOLD_ALERT, JSON.stringify({ remaining: newTotalTablets, threshold }), currentTime]
            );
        }
    }
     ...
     */
    public static async refillCompartment(
        client: PoolClient,
        ...
        const compartmentRes = await client.query(
            `SELECT total_tablets FROM compartments WHERE id = $1 FOR UPDATE`,
            [compartmentId]
        );
       ...
       WHERE id = $3`,
            [newTotal, currentTime, compartmentId]
        );

        await client.query(
            `INSERT INTO audit_logs (compartment_id, event_type, details, created_at) 
       VALUES ($1, $2, $3, $4)`,
            [compartmentId, EventType.REFILL, JSON.stringify({ added: addedTablets, newTotal }), currentTime]
        );
    }
}
